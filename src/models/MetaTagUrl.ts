...
export interface IMetaTagUrl extends Document {
  url: string;
  urlPattern?: string; // For pattern-based rules
  title: string;
  description: string;
  keywords?: string[];
  canonicalUrl?: string;
...
MetaTagUrlSchema.index({ url: 1 });
MetaTagUrlSchema.index({ isActive: 1 });
MetaTagUrlSchema.index({ createdAt: -1 });

export default mongoose.models.MetaTagUrl || mongoose.model<IMetaTagUrl>('MetaTagUrl', MetaTagUrlSchema);